...
/**
 * 整本导出流编排（从 ReaderView 收出）：start / cancel / 状态三态 + 卸载 abort。
 *
...
/** 导出状态三态：空闲 / 运行中（KB 进度 + 章数标题）/ 失败（错误类目投影，与阅读链路 error 分家） */
...
export const IDLE_EXPORT: ExportRunState = { running: false, kb: 0, total: '', error: null }
...
export function createExportRun(ctx: {
  ...
  return {
    ...
    start: (): void => {
      ...
      ctrl = c
      set({ running: true, kb: 0, total: '', error: null })
      void deps.streamExport({
        ...
        deps.saveBlob(blob, `${ctx.title}.txt`)
      }, (e: unknown) => {
