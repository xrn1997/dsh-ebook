...
export interface FollowOptions {
  ...
  /** 目录已知章节地址（legado 口径的防串章判据）：候选 == 其他章节 URL → 停。
...
/**
 * 翻页跟进（legado BookContent/BookChapterList 对齐）：
 * - **next 规则按列表语义求值**（legado getStringList(isUrl=true)）：1 个候选 → 链式跟进
 *   （每页继续求值 next）；多个候选 → 全部抓取但**不递归翻页**（legado getNextPageUrl=false）；
 * - 防环按 **URL 已见**（legado nextUrlList 口径），条目去重按 keyOf；
 * - 判到底三态：本页 0 条 → zero-new（空页之后的页不可信）；本页有条目但 0 新增 → loop
 *   （**部分重复不再停**——legado 目录翻页只按 URL 防环、条目最后统一去重，站点页间重叠是常态，
 *   此前「出现重复条目即停」把重叠的真实页截断）；上限 → cap；
 * - 串章闸：stopUrls（目录知识，legado「下一页 == 下一章 URL → break」）优先，
...
export async function followPages<T>(
  ...
  outer: while (queue.length > 0) {
    ...
    if (!entry.chain) continue // 多 URL 模式：本页不再求值 next（legado getNextPageUrl=false）
    // next 规则：**列表语义**（legado getStringList(isUrl=true)：逐项绝对化 + 去重 + 丢空）
