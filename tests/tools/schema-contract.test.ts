...
/**
 * 工具 schema 契约测试：
 * 五份手写 schema 是工具对 harness 的 interface（additionalProperties:false——execute 多吐一个键、
...
describe('工具 schema 契约（execute 输出 ≡ 声明 schema；schema ≡ wire 字段）', () => {
  it('① 每个工具的 execute 输出都通过 harness 同款 schema 校验', async () => {
    const s = await svc()
    s.shelfAdd('https://s.com/book/1/', { sourceId, title: '斗罗' })
    const calls: Array<[string, unknown]> = [
      ['novel_search_books', { keyword: '斗罗' }],
      ['novel_read_chapter', { sourceId, bookKey: 'https://s.com/book/1/', chapterIndex: 0 }],
      ['novel_add_source', { sourceJson: '[{"bookSourceName":"B","bookSourceUrl":"https://b","ruleContent":"x"}]' }],
      ['novel_add_source', { sourceJson: JSON.stringify(rawSource) }],                 // 同址重复 → dupSkipped 分支也要过 schema
      ['novel_add_source', { sourceJson: '{oops' }],                                  // 坏 JSON 分支也要过 schema
      ['novel_probe_source', { sourceId }],
      ['novel_shelf', { action: 'list' }],
    ]
  ...
  it('② schema 属性集 ≡ wire 类型字段集（表钉死；缺键投影差集=nullable→缺席）', async () => {
    const s = await svc()
    const search = schemaOf(s, 'novel_search_books')
    // ── novel_search_books ≡ SearchGroup/SearchHit（`shared/wire.ts` 的两个 interface）──
    ...
    expect(propsOf(search, 'groups', '[]', 'hits', '[]'))
      .toEqual(new Set(['title', 'author', 'url', 'coverUrl', 'intro', 'lastChapterName']))
    ...
    // ── novel_read_chapter：随读工具自己的规范值（sourceId/bookKey/chapterIndex/chapterName/text）──
    expect(propsOf(schemaOf(s, 'novel_read_chapter')))
      .toEqual(new Set(['sourceId', 'bookKey', 'chapterIndex', 'chapterName', 'text']))

    // ── novel_add_source ≡ ImportOutcome 投影（name/ok/sourceId/missing/warnings + 按址去重 dupSkipped）──
    expect(propsOf(schemaOf(s, 'novel_add_source'), 'outcomes', '[]'))
      .toEqual(new Set(['sourceId', 'name', 'ok', 'dupSkipped', 'missing', 'warnings']))

    // ── novel_probe_source ≡ ProbeResult + sourceId/status（`shared/wire.ts` 的 ProbeResult）──
    expect(propsOf(schemaOf(s, 'novel_probe_source')))
      .toEqual(new Set(['sourceId', 'status', 'ok', 'itemCount', 'firstTitle', 'error', 'probedAt']))

    // ── novel_shelf ≡ ShelfBook 投影（`shared/wire.ts` 的 ShelfBook；intro/totalChapters 故意不在 schema——超集会被拒）──
    ...
    expect(propsOf(schemaOf(s, 'novel_shelf'), 'books', '[]'))
      .toEqual(new Set([
  ...
  it('③ 五份 schema 本体都在 harness 强制子集内（assertSupportedJsonSchema 不抛）', async () => {
    const s = await svc()
